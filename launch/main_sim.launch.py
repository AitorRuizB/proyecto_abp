...
def launch_setup(context, *args, **kwargs):
    ...
    nodes.append(Node(
        ...
        name='global_bridge',
        parameters=[{'config_file': global_bridge_path}],
        output='screen'
    ))
    ...
    for i in range(1, num_robots + 1):
        ...
        nodes.append(Node(
            ...
            arguments=['-name', robot_name, '-string', robot_desc,
                       '-x', str(x_pose), '-y', str(y_pose), '-z', '0.2'],
            output='screen'
        ))

        # Bridge local por robot
        ...
        nodes.append(Node(
            package='ros_gz_bridge', executable='parameter_bridge',
            name=f'bridge_{robot_name}', namespace=robot_name,
            parameters=[{'config_file': local_bridge_path}],
            output='screen'
        ))

        # SLAM Toolbox (LifecycleNode + lifecycle_manager con bond_timeout 0.0)
        ...
        nodes.append(Node(
            ...
            parameters=[{
                ...
                'bond_timeout': 0.0,  # sin timeout no falla al arrancar
            }],
            output='screen'
        ))
        ...
        nodes.append(Node(
            ...
            arguments=[
                '--x', str(x_pose), '--y', str(y_pose), '--z', '0.0',
    ...
    # ── 4. MAP MERGE (custom_map_merger.py) ───────────────────────────────────
    nodes.append(TimerAction(
        period=20.0,
        actions=[Node(
            package='proyecto_abp',
            executable='map_merge',      # entry point en setup.py
    ...
    nodes.append(Node(
        ...
        arguments=['-d', os.path.join(pkg, 'rviz', 'config.rviz')],
